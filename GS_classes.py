...
class G1():
        ...
        for i in range(len(intable)):
            with open(intable[i], 'r') as infile:
                ...
                with open(outputdir+'/'+contrast+'_scaf_'+str(i+1)+'.tab','w') as outfile:
                    ...
                    print('Retained '+str(count-Zero_count-All_count-non_variant_count)+' variants\n')
                    ...
                    with open(logdir+contrast+'_scaf_'+str(i+1)+'_inputfilter.csv', 'w') as logfile:
                        ...
                        logfile.write('Total variants in file,'+str(count)+'\n')
                        logfile.write('Variants with total allele count = 0,\t'+str(Zero_count)+'\n')
                        logfile.write('Variants fixed in both cohorts,'+str(All_count)+'\n')
                        logfile.write('Other non-variant sites,'+str(non_variant_count)+'\n')
                        logfile.write('Retained variants,'+str(count-Zero_count-All_count-non_variant_count))
    ...
    def step3(self):
        ...
        with open(outputdir+'/'+contrast+'_WG_ACs'+args.suf+'.txt','w') as outfile:
    ...
    def step4(self):
        ...
        header = 'scaffold\tstart\tend\tmidpoint\tlength\t'+args.coh1+'_freq\t'+args.coh2+'_freq\tabsdiff\t'+args.coh1+'_'+args.coh2+'\t'+args.coh2+'_'+args.coh1+'\tvaru\t'+args.coh1+'_pi\t'+args.coh2+'_pi\t'+args.coh1+'_thetaw\t'+args.coh2+'_thetaw\t'+args.coh1+'_thetah\t'+args.coh2+'_thetah\tFst\tFst_4C\tDxy\n'
    ...
    def step5(self):
        ...
        for out in out_sort:
            with open(out, 'r') as infile:
                ...
                for line in infile:
                    data = line.split()
                    # scaffold, start, end, midpoint, length, '+args.coh1+'_freq, '+args.coh2+'_freq, absdiff, '+args.coh1+'_'+args.coh2+', '+args.coh2+'_'+args.coh1+', varu, '+args.coh1+'_pi, '+args.coh2+'_pi, '+args.coh1+'_thetaw, '+args.coh2+'_thetaw, '+args.coh1+'_thetah, '+args.coh2+'_thetah, Fst, Dxy, prediction, cor_prediction, DD, tajimas_D_'+args.coh1+', tajimas_D_'+args.coh2+', faiwus_H_'+args.coh1+', faiwus_H_'+args.coh2+' = line.split()
                    absdiff.append(float(data[7]))
                    afd_c1_c2.append(float(data[8]))
                    afd_c2_c1.append(float(data[9]))
                    pi_c1.append(float(data[11]))
                    pi_c2.append(float(data[12]))
                    thetaw_c1.append(float(data[13]))
                    thetaw_c2.append(float(data[14]))
                    thetah_c1.append(float(data[15]))
                    thetah_c2.append(float(data[16]))
                    fst.append(float(data[17]))
                    fst_4C.append(float(data[18]))
                    dxy.append(float(data[19]))
                    dd.append(float(data[22]))
                    tajD_c1.append(float(data[23]))
                    tajD_c2.append(float(data[24]))
                    faiwuH_c1.append(float(data[25]))
                    faiwuH_c2.append(float(data[26]))
                ...
                mean_abs = statistics.mean(absdiff)
                median_abs = statistics.mean(absdiff)
                sd_abs = statistics.stdev(absdiff)
                CV_abs = sd_abs/mean_abs
                out_abs_up = np.percentile(absdiff, (100-args.cut))
                out_abs_low = np.percentile(absdiff, args.cut)
                ...
                outlier_values.write('stat\tAbsDiff\tAFD_1_2\tDxy\tFst\tFst_4C\tDD\twin_length\tTajD_'+args.coh1+'\tTajD_'+args.coh2+'\tFaiWusH_'+args.coh1+'\tFaiWusH_'+args.coh2+'\n'+
                                     'min\t'+str(round(min(absdiff),4))+'\t'+str(round(min(afd_c1_c2),4))+'\t'+str(round(min(dxy),4))+'\t'+str(round(min(fst),4))+'\t'+str(round(min(fst_4C),4))+'\t'+str(round(min(dd),4))+'\t'+str(round(min(length),0))+'\t'+str(round(min(tajD_c1),4))+'\t'+str(round(min(tajD_c2),4))+'\t'+str(round(min(faiwuH_c1),4))+'\t'+str(round(min(faiwuH_c2),4))+'\n'+
                                     ...
                                     'max\t'+str(round(max(absdiff),4))+'\t'+str(round(max(afd_c1_c2),4))+'\t'+str(round(max(dxy),4))+'\t'+str(round(max(fst),4))+'\t'+str(round(max(fst_4C),4))+'\t'+str(round(max(dd),4))+'\t'+str(round(max(length),0))+'\t'+str(round(max(tajD_c1),4))+'\t'+str(round(max(tajD_c2),0))+'\t'+str(round(max(faiwuH_c1),4))+'\t'+str(round(max(faiwuH_c2),0))+'\n'+
