...
## updated 12 June 2018 by Christian Sailer
...
class G_NullW():
	...
	def step1_nullW(self):
		...
		for dirName, subdirList, fileList in os.walk(args.i+contrast):
			for file in fileList:
				if file.startswith(contrast) and file.endswith('WG_AC'+args.suf+'.txt'):
					ac.append(file)
	...
	def step3_NullW(self):
		...
		# subset with candidate gene list
		if args.o == 'na':
			outdir = outputdir.replace('perSNP', 'genes')
		else:
			outdir = args.o+contrast+'/genes/'

		# find candidate gene intervals, from step DS2
		cand_list = []
		for dirName, subdirList, fileList in os.walk(outdir):
			for file in fileList:
				if args.outlier in file and file.endswith('intervals'+args.suf+'.bed'):
					cand_list.append(outdir+file)
	...
	def step4_NullW(self):
		...
		# get annotated vcf
		for dirName, subdirList, fileList in os.walk('Data/ann_vcf/'):
			for file in fileList:
